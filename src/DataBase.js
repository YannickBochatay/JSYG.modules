require([ "JSYG", "StdConstruct", "Promise" ], function(JSYG) {
	...
	JSYG.DataBase.prototype.createQuery = function() {
		...
		return new JSYG.DataBase.Query(this._db, this._objectStoreName);
	};
	...
	JSYG.DataBase.prototype.open = function() {
		...
		if (!indexedDB)
			throw new Error(
					"IndexedDB n'est pas implémenté dasn votre navigateur");
		if (!this._name)
			throw new Error("Aucun nom de base n'a été défini");

		var request = indexedDB.open(this._name, this.version), that = this;
	...
	JSYG.DataBase.prototype["delete"] = function() {
		...
		var request = indexedDB.deleteDatabase(this._name), that = this;
	...
	JSYG.DataBase.Query = function(db, storeObjectName) {
		...
		if (!storeObjectName)
			throw new Error("Il faut définir un nom de storeObject");
	...
	JSYG.DataBase.Query.prototype.get = function(uniqueID) {
		...
		var transaction = this._db.transaction(this._storeObjectName),
			...
			keyRange, cursor, request, resul = [], methodNext, that = this;
		...
		if (this._bounds.min != null && this._bounds.max != null)
			keyRange = IDBKeyRange.bound(this._bounds.min,
					this._bounds.max, this._bounds.minStrict || false,
					this._bounds.maxStrict || false);
		else if (this._bounds.min != null) {
			keyRange = IDBKeyRange.lowerBound(this._bounds.min,
					this._bounds.minStrict || false);
		} else if (this._bounds.max != null) {
			keyRange = IDBKeyRange.upperBound(this._bounds.max,
					this._bounds.maxStrict || false);
		}

		methodNext = (this._desc ? "prev" : "next")	+ (this._distinct ? "unique" : "");

		cursor = (index ? index : objectStore).openCursor(keyRange,
				methodNext);
		...
		return new JSYG.Promise(function(resolve, reject) {
			...
			transaction.oncomplete = function() {
				that._db.trigger("get", that._db, resul);
				that.trigger("get", objectStore, resul);
				resolve(resul);
			};

			transaction.onerror = function(e) {
				that._db.trigger("error", that._db, e);
				that.trigger("error", objectStore, e);
				reject(e);
			};
		});
